...
class TZ_Guard_Browser_Detect {
		...
		*/
		if ( !$b_repeat )
		{
			...
			*/
					...
					switch ( $browser_name )
					{
						...
						case 'moz':
							...
							*/
							...
							$moz_rv_full = $this->browser_version( $browser_user_agent, 'rv' );
							...
							for ( $i = 0; $i < $i_count; $i++ )
							{
								if ( stristr( $browser_user_agent, $moz_types[$i] ) )
								{
									...
									$moz_version_number = $this->browser_version( $browser_user_agent, $moz_version );
									break;
								}
							}
							...
							$moz_release = $this->browser_version( $browser_user_agent, 'gecko/' );
							/*
							...
							*/
							...
							break;
						case 'ie':
							$version_number = $this->browser_version( $browser_user_agent, $s_browser );
						...
						case 'webkit':
							...
							for ( $i = 0; $i < $i_count; $i++ )
							{
								if ( stristr( $browser_user_agent, $webkit_types[$i] ) )
								{
									...
									$webkit_version_number = $this->browser_version( $browser_user_agent, $webkit_version );
									// epiphany hack
			...
			$a_os_data = $this->which_os( $browser_user_agent, $browser_name, $version_number );
	...
	function which_os ( $browser_string, $browser_name, $version_number  )
	{
		...
		*/
		...
		for ( $i = 0; $i < $i_count; $i++ )
		{
			...
			{
				$i_count_unix = count($s_os);
				for ($j = 0; $j < $i_count_unix; $j++)
				{
			...
			{
				$i_count_linux = count($s_os);
				for ($j = 0; $j < $i_count_linux; $j++)
				{
					if ( stristr( $browser_string, $s_os[$j] ) )
					{
						$os = 'lin';
	...
	//function $this->browser_version( $browser_user_agent, $search_string, $substring_length )
